...
def captureWindowClose():
    """Capturar cierre de ventana para cerrar el juego"""
...
while isRunning:
    # Capturar cierre de ventana para cerrar el juego
    isRunning = captureWindowClose()
    
    # Si pulsamos la tecla de salida
    if pygame.key.get_just_pressed()[keyExit]:
    ...
    if inTitle:
        # Dibujamos el título
        drawTitle()
        # Si pulsamos la tecla de inicio
        if pygame.key.get_just_pressed()[keyFire]:
    ...
    # Si no, estamos en el juego
        ...
        # Capturar pausa
        if pygame.key.get_just_pressed()[keyPause]:
            inPause = not inPause
        ...
        # Si estamos en juego
            ...
            # Si se pulsa la tecla de inicio disparar
            if pygame.key.get_just_pressed()[keyFire]:
                fireSfx.play()
                shots += 1
